...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3 pb-4 border-b border-slate-800">
        ...
        <div>
            <h1 class="orbitron-title text-xl font-black text-slate-100 tracking-wider">EDIT BANK ACCOUNT</h1>
            <p class="text-slate-400 text-xs mt-0.5 uppercase tracking-widest font-semibold font-sans">Modify corporate bank transfer destination details</p>
        </div>
    </div>

    <!-- Form Panel -->
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <form action="{{ route('bank_accounts.update', $bankAccount->id) }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="bank_name" value="{{ old('bank_name', $bankAccount->bank_name) }}" placeholder="e.g. Commercial Bank" required class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="account_number" value="{{ old('account_number', $bankAccount->account_number) }}" placeholder="e.g. 1090012345" required class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors font-mono">
                </div>

                <div>
                    <label class="text-[10px] text-slate-400 uppercase tracking-wider font-bold block mb-1">Branch Name</label>
                    <input type="text" name="branch" value="{{ old('branch', $bankAccount->branch) }}" placeholder="e.g. Colombo 03" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="is_active" required class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                    <option value="1" {{ old('is_active', $bankAccount->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>Active (Available for transactions)</option>
                    <option value="0" {{ old('is_active', $bankAccount->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>Inactive (Hidden during checkout)</option>
                </select>
            </div>
