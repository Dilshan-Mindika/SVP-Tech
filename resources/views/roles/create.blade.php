...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between pb-4 border-b border-slate-800">
        <div>
            <h1 class="orbitron-title text-xl font-black text-slate-100 tracking-wider">CREATE SYSTEM ROLE</h1>
            <p class="text-slate-400 text-xs mt-0.5 uppercase tracking-widest font-semibold font-sans">Establish a new role and configure its authorization schema</p>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-1">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Sales Executive" required class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500">
                </div>
                <div class="md:col-span-2">
                    <label class="text-[10px] text-slate-400 uppercase tracking-wider font-bold block mb-1">Description / Notes</label>
                    <input type="text" name="description" value="{{ old('description') }}" placeholder="Brief summary of responsibility permissions" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500">
                </div>
            </div>
        </div>
                        ...
                        <div class="space-y-2" id="module-{{ Str::slug($module) }}">
                            @foreach($modPermissions as $perm)
                                <div class="flex items-center justify-between py-0.5">
                                    ...
                                    <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" id="perm-{{ $perm->id }}" class="rounded border-slate-800 bg-slate-900 text-cyan-500 focus:ring-cyan-500">
                                </div>
                            @endforeach
                        </div>
                    </div>
