...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3 pb-4 border-b border-slate-800">
        ...
        <div>
            <h1 class="orbitron-title text-xl font-black text-slate-100 tracking-wider">REGISTER NEW EMPLOYEE</h1>
            <p class="text-slate-400 text-xs mt-0.5 uppercase tracking-widest font-semibold font-sans">Input personnel profile details and assign Laravel authentication linkages</p>
        </div>
    </div>
        ...
        <form action="{{ route('employees.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="Enter full legal name" value="{{ old('name') }}" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                </div>
                ...
                <div>
                    ...
                    <select name="designation" required class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                        ...
                        <option value="Technician" {{ old('designation') === 'Technician' ? 'selected' : '' }}>Technician</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="salary_amount" required step="0.01" min="0" placeholder="e.g. 3500.00" value="{{ old('salary_amount') }}" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors font-mono">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="joining_date" required value="{{ old('joining_date', date('Y-m-d')) }}" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                </div>

                <!-- Phone -->
                <div>
                    <label class="text-[11px] text-slate-400 uppercase tracking-wider font-bold block mb-1">Mobile / Phone Number</label>
                    <input type="text" name="phone" placeholder="e.g. +1 555-0199" value="{{ old('phone') }}" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors font-mono">
                </div>

                <!-- Email -->
                <div>
                    <label class="text-[11px] text-slate-400 uppercase tracking-wider font-bold block mb-1">Work Email Address</label>
                    <input type="email" name="email" placeholder="e.g. employee@cloudtech.com" value="{{ old('email') }}" class="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="user_id" class="w-full md:w-1/2 bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-cyan-500 transition-colors">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id') == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }}) [Role: {{ $user->roleRelation ? $user->roleRelation->name : 'No Role' }}]</option>
                        @endforeach
                    </select>
                </div>
            </div>
